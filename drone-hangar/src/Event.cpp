...
void InterruptDispatcher::detach(int pin){
  disableInterrupt(pin);
  sourceRegisteredOnPin[pin] = nullptr;
}

void InterruptDispatcher::notifyInterrupt(int pin){
  if (sourceRegisteredOnPin[pin] != nullptr) {
...
void EventSource::detachInterrupt(int pin){
  interruptDispatcher.detach(pin);
}
